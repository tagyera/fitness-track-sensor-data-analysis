...
for col in predictor_columns:
    df_lowpass = LowPass.low_pass_filter(
        df_lowpass, col, fs, cutoff, order=5, phase_shift=True
    )
...
ws = int(1000 / 200)  # taking ws for 1 sec , which is 1000ms
...
for s in df_freq["set"].unique():
    ...
    subest = FreqAbs.abstract_frequency(subset, predictor_columns, ws, fs)
...
fig = plt.figure(figsize=(15, 15))
ax = fig.add_subplot(projection="3d")
for la in df_cluster["label"].unique():
    subset = df_cluster[df_cluster["label"] == la]
    ax.scatter(subset["acc_x"], subset["acc_y"], subset["acc_z"], label=la)
ax.set_xlabel("X-axis")
ax.set_ylabel("Y-axis")
ax.set_zlabel("Z-axis")
plt.legend()
